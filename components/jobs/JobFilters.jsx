...
export const JobFilters = ({ location, setLocation, category, setCategory, filters, setFilters, counts, onClearAll, payRange = [0, 200], setPayRange }) => {
                ...
                <FilterSection title="Pay Rate ($/hr)">
                    <div className="px-1">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold text-slate-700">${counts.minPay || 0}</span>
                            <span className="text-xs font-bold text-slate-700">${counts.maxPay || 200}+</span>
                        </div>
                        <div className="relative h-10 w-full group">
                            ...
                            <div className="absolute top-1/2 left-0 w-full h-1.5 bg-slate-100 rounded-full -translate-y-1/2 overflow-hidden">
                                ...
                                <div
                                    ...
                                    style={{
                                        left: `${(payRange[0] / 200) * 100}%`,
                                        width: `${((payRange[1] - payRange[0]) / 200) * 100}%`
                                    }}
                                />
                            </div>
                            ...
                            <input
                                ...
                                value={payRange[0]}
                                onChange={(e) => {
                                    const val = Math.min(Number(e.target.value), payRange[1] - 5);
                            ...
                            <input
                                ...
                                value={payRange[1]}
                                onChange={(e) => {
                                    const val = Math.max(Number(e.target.value), payRange[0] + 5);
